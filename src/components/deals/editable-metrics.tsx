...
export function EditableMetrics({ deal }: { deal: Deal }) {
  ...
  const inPlaceCap = deal.current_noi && deal.asking_price > 0
    ...
    : null;
  const occupancy = deal.current_occupancy;
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Financing & NOI quick metrics — shown when data is available */}
      {(inPlaceCap !== null || deal.loan_amount || occupancy !== undefined) && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {inPlaceCap !== null && (
            <Card className="bg-slate-900 border-slate-800">
              <CardContent className="pt-4 pb-3 px-4">
                <p className="text-xs text-slate-500 mb-1">In-Place Cap</p>
                <p className="text-lg font-bold text-white">{(inPlaceCap * 100).toFixed(2)}%</p>
              </CardContent>
            </Card>
          )}
          {deal.current_noi && (
            <Card className="bg-slate-900 border-slate-800">
              <CardContent className="pt-4 pb-3 px-4">
                <p className="text-xs text-slate-500 mb-1">Current NOI</p>
                <p className="text-lg font-bold text-white">{formatCurrency(deal.current_noi)}</p>
              </CardContent>
            </Card>
          )}
          {occupancy !== undefined && occupancy !== null && (
            <Card className="bg-slate-900 border-slate-800">
              <CardContent className="pt-4 pb-3 px-4">
                <p className="text-xs text-slate-500 mb-1">Occupancy</p>
                <p className="text-lg font-bold text-white">{(occupancy * 100).toFixed(0)}%</p>
              </CardContent>
            </Card>
          )}
          {deal.loan_amount && (
            <Card className="bg-slate-900 border-slate-800">
              <CardContent className="pt-4 pb-3 px-4">
                ...
                <p className="text-lg font-bold text-white">{formatCurrency(deal.loan_amount)}</p>
              </CardContent>
            </Card>
          )}
          {deal.monthly_debt_service && (
            <Card className="bg-slate-900 border-slate-800">
              <CardContent className="pt-4 pb-3 px-4">
                <p className="text-xs text-slate-500 mb-1">Monthly P&I</p>
                <p className="text-lg font-bold text-white">{formatCurrency(deal.monthly_debt_service)}</p>
              </CardContent>
            </Card>
          )}
        </div>
